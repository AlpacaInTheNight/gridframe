...
class GridPanel extends Component {
    constructor(props) {
        ...
        this.showOptions = () => {
            const options = [];
            options.push(createElement("option", { key: "0" }));
            for (const index in this.context.components) {
                const component = this.context.components[index];
                options.push(createElement("option", { key: index, value: index }, component.name ? component.name : index));
            }
            return options;
        };
    ...
    render() {
        ...
        return (createElement("div", { className: classPrefix + "controlPanel", style: panelStyle },
            createElement("div", { style: splitStyle, "data-grabber": "true", className: classPrefix + "spliter", onMouseDown: this.onGrabberStart }),
            createElement("select", { className: classPrefix + "componentSelector", style: selectorStyle, onChange: this.onChangeComponentId, "data-id": this.props.elementId, value: this.props.componentId || "" }, this.showOptions()),
            createElement("div", { style: swapStyle, "data-grabber": "true", className: classPrefix + "swapper", onMouseDown: this.onSwapStart }),
            createElement("div", { style: joinStyle, "data-grabber": "true", className: classPrefix + "joiner", onMouseDown: this.onJoinerStart })));
    }
}
...
class GridElement extends Component {
    constructor(props) {
        ...
        this.showOverlay = (props) => {
            ...
            return createElement("div", { style: overlayStyle, className: className });
        };
        this.getHTMLId = () => {
            const { gridAreaId } = this.context.getWorkArea();
            const { element } = this.props;
    ...
    render() {
        ...
        return (createElement("div", { style: cellStyle, key: "gridElement:" + element.id, "data-id": element.id, className: classPrefix + "container " + addClass, id: htmlId, onMouseEnter: this.onGridContainerEnter, onMouseLeave: this.onGridContainerLeave, onMouseMove: this.onGridContainerMove, onDragStart: this.onContainerDrag, onDrop: this.onContainerDrop, onDragOver: this.onDragOver, draggable: true },
            joinStatus !== "none" &&
                createElement(this.showOverlay, { status: joinStatus }),
            this.context.showPanel &&
                createElement(GridPanel, { elementId: element.id, componentId: this.props.element.componentId || "" }),
            componentContainer &&
                createElement(GridContainer, { body: componentContainer.body, props: componentContainer.props, containerId: element.id, htmlContainerId: htmlId, changeComponentId: this.context.changeComponentId, adaptiveObserve: adaptiveObserve })));
    }
}
